...
/**
 * cat_cd - function that concatenates the message for cd error
 *
 * @datahsh: data relevant (directory)
 * @msg: message to print
 * @error: output message
 * @ver_str: counter lines
 * Return: error message
 */
char *cat_cd(data_shell *datahsh, char *msg, char *error, char *ver_str)
{
	char *illegal_flag;
 ...
 * Return: Error message
 */
char *error_get_cd(data_shell *datahsh)
{
	int length, len_id;
	char *error, *ver_str, *msg;

	ver_str = aux_itoa(datahsh->counter);
	if (datahsh->args[1][0] == '-')
	{
		msg = ": Illegal option ";
		len_id = 2;
	...
	error = cat_cd(datahsh, msg, error, ver_str);
 ...
 * Return: Error message
 */
char *error_not_found(data_shell *datahsh)
{
	int length;
	char *error;
	char *ver_str;
 ...
 * Return: Error message
 */
char *error_exit_shell(data_shell *datahsh)
{
	int length;
	char *error;
	char *ver_str;
